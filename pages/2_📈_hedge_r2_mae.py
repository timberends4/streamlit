...
if "df" not in st.session_state or "product_options" not in st.session_state:
    ...
    st.date_input(
        "Select start date",
        value = pd.to_datetime(json_prices["date"].min()).date(),
        min_value = pd.to_datetime(json_prices["date"].min()).date(),
        max_value = pd.to_datetime(json_prices["date"].max()).date(),
        key="_start_date",
    ...
    st.date_input(
        "Select end date",
        value = pd.to_datetime(json_prices["date"].max()).date(),
        min_value = pd.to_datetime(json_prices["date"].min()).date(),
        max_value = pd.to_datetime(json_prices["date"].max()).date(),
        key="_end_date",
        on_change=store_value,
    ...
    if "results_r2" in st.session_state and "results_mae" in st.session_state:
        ...
        if selected_f1_percentage in st.session_state.residuals_dict:
            ...
            bin_edges = np.arange(-2, 2, 0.02)  # Bins from -1000 to 1000 with step size 50

            # Create histogram data
            hist_values, _ = np.histogram(selected_residuals, bins=bin_edges)

            # Create DataFrame for Streamlit bar chart
            bin_centers = bin_edges[:-1] + 0.01  # Calculate bin centers for better visualization
            histogram_df = pd.DataFrame({
                "Residuals Frequency": hist_values,
                "Bins": bin_centers.round(2)  # Round bin centers to 2 decimal places
            })
