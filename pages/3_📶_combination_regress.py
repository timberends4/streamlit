...
def run_regression():
    ...
    if not df.empty and all([p1_product, p2_product, y_product, start_date, end_date]):
        ...
        df_filtered = df[
            ((df['product_sc'].isin([p1_product, p2_product])) |
            (df['product_sc'].isin(y_product))) &
        ...
        total_y_features = sum(
            [st.session_state.get(f"weight_{feat}", 0) for feat in st.session_state.get("y_row_1", [])]
        )

        y = sum(df_merged[feat] * y_weights.get(feat)/total_y_features for feat in y_selected)
        df_merged['y'] = y

        st.session_state['df_pivot_merged'] = df_merged  # Update session state with filtered data
...
if st.session_state.get("df") is not None:
    ...
    with col1:
        st.write("Y")
        y_selected = st.multiselect("Select Y products", product_options, key="y_row_1")
        y_weights = {}
        for y_feat in y_selected:
            y_weights[y_feat] = st.number_input(f"Weight for {y_feat}", min_value=0.0, max_value=1.0, value=1.0, key=f"weight_{y_feat}")
    ...
    df_merged_pivot = st.session_state.get('df_pivot_merged')
    products_y = st.session_state.get("y_row_1", [])

    all_cols_in = all(col in df_merged_pivot.columns for col in products_y)

    if not all_cols_in:
        st.warning("Run the selected model with updated products to see the results.")

    else:
        df_merged_pivot = st.session_state.get('df_pivot_merged', pd.DataFrame())
        df_merged_pivot['date'] = pd.to_datetime(df_merged_pivot['date'])
        ...
        if 'predictions' in st.session_state:
            ...
            y = sum(df_merged_pivot[feat] * y_weights.get(feat)/total_y_features for feat in y_selected)
            pred_df['combination'] = y
            ...
            if 'date' in pred_df.columns:
                ...
                if not pred_df.empty:
                    pred_df['Spread'] = pred_df['combination'] - pred_df['predicted_price']
                    pred_df['Spread'] = pred_df['Spread'].dropna()

                    st.session_state['Spread'] = pred_df['Spread']

                    y_actual = pred_df['combination'].values
                    y_pred = pred_df['predicted_price'].values
                    pred_dates = pred_df['date']
                    ...
                    pred_df['Positive_Spread'] = pred_df['Spread'].apply(lambda x: x if x > 0 else 0)
                    pred_df['Negative_Spread'] = pred_df['Spread'].apply(lambda x: x if x < 0 else 0)

                    # Add Positive Spread Area (Green)
                    fig.add_trace(go.Scatter(
                        x=pred_df['date'],
                        y=pred_df['Positive_Spread'],
                        mode='lines',
